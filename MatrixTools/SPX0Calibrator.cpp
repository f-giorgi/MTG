//SPX0 Calibrator.
// usage:
// ./SPX0Calibrator datafileRNXXX.dat
...
const unsigned int col_steps = 32;

  
unsigned long evt_n       = 0; //number of events found
unsigned long corrupt_evt = 0; // number of corrupted event found  
unsigned long NnonTriggerEvents = 0;

//const float dac_to_mV = 1250./4096;
	
int main(int argc, char* argv[]){
  if (argc <= 1 ){
    cout << "Usage: SPX0Calibrator [-v0|1|2] [-evtmax|-e n_of_events] [-rob 1|0] [-purify | -sanitize] [-local triggers DACstart DACstop DACstep Layers nType=3)] <eventfile>" << endl;     
    return -1;
  }
  cout<<"+++++++++++++++SPX0 Calibrator 0.1 by F. Giorgi+++++++++++++++++++++++++"<<endl;
  cout << "Usage: SPX0Calibrator [-v0|1|2] [-evtmax|-e n_of_events] [-rob 1|0] [-purify | -sanitize] [-local triggers DACstart DACstop DACstep Layers nType=3] <eventfile>" << endl<<"INFO:: -purify skip hits that belongs to wrong columns w.r.t. current col-step"
  ...
  if (SQLmode){
    cout<< "sqlMODE retrieving parameters"<<endl;
    EPMCcalpar1.Reset();
    EPMCcalpar2.Reset();
    sqlselect(run_number, EPMCcalpar1, EPMCcalpar2);
	cout<< "sqlMODE select succesful"<<endl;
    if(      EPMCcalpar1.type0==0x0 && EPMCcalpar1.layers==0x1){
      FEMcalpar = EPMCcalpar1.fem0_cp;
      Layers = EPMCcalpar1.layers;
    } else if (EPMCcalpar1.type1==0x0 && EPMCcalpar1.layers==0x2){
      FEMcalpar = EPMCcalpar1.fem1_cp;
      Layers = EPMCcalpar2.layers;
    } else if (EPMCcalpar2.type0==0x0 && EPMCcalpar2.layers==0x10){
      FEMcalpar = EPMCcalpar2.fem0_cp;
      Layers = EPMCcalpar2.layers;
    } else if (EPMCcalpar2.type1==0x0 && EPMCcalpar2.layers==0x20){
      FEMcalpar = EPMCcalpar2.fem1_cp;
      Layers = EPMCcalpar2.layers;
    } else {
      cout<<"ERROR! SQL returned no active SPX0 pixel layer "<<endl;
      return -1;
    }
  ...
  char rootfile[200];
  if(purify)
    snprintf(rootfile,199,"HistosSPX0Calib_%07d_purified.root",run_number);
  else if (sanitize)
    snprintf(rootfile,199,"HistosSPX0SCalib_%07d_sanitized.root",run_number);
  else
    snprintf(rootfile,199,"HistosSPX0Calib_%07d.root",run_number);
...
if(verbosity==kDEBUG||verbosity==kMEDIUM) cout<<"Root data file opened"<<endl;
  ...
  // Final statistics
  unsigned int expected_evt = FEMcalpar.triggers*col_steps*(FEMcalpar.dacstop - FEMcalpar.dacstart + FEMcalpar.dacstep )/FEMcalpar.dacstep ;


  cout << evt_n             <<"\t\t GOOD events, of which "<<NnonTriggerEvents<<" non-trigger events --> "<<evt_n-NnonTriggerEvents<<"  events with trigger" <<endl;
  cout << corrupt_evt       <<"\t\t BAD  events"<<endl;
  cout <<"==========="<<endl;
  cout << evt_n+corrupt_evt <<"\t\t TOT  event fragments"<<endl<<endl;

  cout << expected_evt      <<"\t\t Expected  triggers"<<endl;
...
int new_hMatrix(int DACidx){
  ...
  hmatrixcurr = new TH2I(name,title,128,-0.5,127.5, 32,-0.5,31.5);
  return 0;
};
...
int FillHist(_EVENT & evt, const FEMCalibParam & calpar,unsigned int layers, _VERBOSITY verbosity){
  ...
  static double meanNhit = 0.0;
  static int NeventXCalStep = 0;
  static int NhitXCalStep= 0;
  ...
  // static int NnonTriggerEvents = 0;
  ...
  if( evt.number_of_robs()>0 && evt.ROBs[ROB].rod.layers.size()>0 ){
	...
	for(unsigned int lay=0; lay<myrod->layers.size(); ++lay){
	  ...
	  if( ((layers>>mylay)&1)==1 ){
		...
		if (myrod->layers[lay].hits[nh].raw == myrod->layers[lay].hits[i].raw && i!=nh )
		  cout<<"Identical hits in event N: "<<evt.ordinal<<"idx1 = "<<nh<<"idx2 = "<<i<<endl;
	      }
		...
		if(chipHit &inspBit) chipHitBit->Fill(bit); 
	      ...
	      // FIRST HIT
	      if( nh == 1) {                 //e' la prima hit 
		if (PIXEL::isEoE(raw) ){     //se e' un trigger    
		  ...
		  //EOS_DACstep	          =(EOS_data.DACvalue-calpar.dacstart)/calpar.dacstep;	//forse non serve	 
		} else {                     //se e' una hit
		  if (verbosity == kDEBUG) cout<<"First hit in event "<< evt.ordinal <<" is not a trigger. Prev. event hit number: "<< prev_Nhit<< " Curr event hit number: "<<nhit<<" Sum:"<<nhit + prev_Nhit<<endl;
		  triggerEvent = false;
		  NnonTriggerEvents++;
		  //controlla che sia la seconda parte di un trigger precedente che ha ecceduto le 64 hit per evento
		  if (prev_Nhit<65)    {  //condizione invalida
		    cout<<"WARNING: First hit of Event "<<evt.ordinal<<" is not a Trigger and prev event length was " << prev_Nhit <<endl; 
		    cout<<"         word parity "<< PIXEL::EvalParity(raw)<<" parity bit: "<<PIXEL::GetParity(raw)<<endl;
		  }
		  if (PIXEL::GetPxIdx(raw)!= 0){
	      ...
	      } else { // tutte le hit successive alla 1
		if (PIXEL::isEoE(raw) ){
		  trigger_counter++;
	    ...
	    if (triggerEvent) NeventXCalStep++;
	    NhitXCalStep += nhit;
	    meanNhit = NhitXCalStep/NeventXCalStep;
	    

	    // oppure meanNhit = NhitXCalStep/trigger_counter;

	    // set trigger idx  with counter value found in data
	    CalIdx.trigidx =  trigger_counter;
	    ...
	    if (triggerEvent && int(EOS_data.DACvalue) > prev_DACvalue) {
	      CalIdx.dacidx++; //first time this goes to 0.
	      CalIdx.colidx=0; //reset the column index
	      cout<<"new DAC value "<<EOS_data.DACvalue<<" reconstructed DAC idx"<<CalIdx.dacidx<<endl;
	      prev_DACvalue = EOS_data.DACvalue;
	      if(verbosity==kDEBUG||verbosity==kMEDIUM) cout<<"create new hMatrix histogram"<<std::endl;
	      new_hMatrix(CalIdx.dacidx);
	      if(verbosity==kDEBUG||verbosity==kMEDIUM) cout<<"fill in hbco"<<std::endl;
	      hbco1->SetBinContent(CalIdx.dacidx+1, exactbco-prev_exactbco);//+1 -> bin(0) is the underflow bin!!
	      prev_exactbco = exactbco;
	      DACstep_BCO->SetBinContent(CalIdx.dacidx+1, exactbco); //+1 -> bin(0) is the underflow bin!!
	    }
	    ...
	    while (!pxl_ids.empty()) {
	      ...
	      if (pxl_ids.back()==33)
		hDACTrigResponsePx33->Fill(CalIdx.dacidx,CalIdx.trigidx%calpar.triggers);
	      pxl_ids.pop_back();
	      col_ids.pop_back();
	      row_ids.pop_back();
	    ...
	    if (triggerEvent) hevts->Fill(CalIdx.colidx,CalIdx.dacidx); 	
	   

	    //find col_idx 
	    if (trigger_counter% calpar.triggers == 0)
	      {
		cout <<"        Ended Col. step: "<< CalIdx.colidx<<" N hit: "<<nhit<<" Nhit max = "<<Nhit_MAX;
		cout <<"   Mean Nhit "<<meanNhit<<endl;
		hmeanNhitXStep->SetBinContent(CalIdx.colidx+1,CalIdx.dacidx+1, meanNhit);//+1 -> bin(0) is the underflow bin!!
		NeventXCalStep = 0;
		NhitXCalStep=0;
		CalIdx.colidx++;
	      }
	    ...
	    //find col_idx 
	    // if (trigger_counter%FEMCalibParam.triggers == 0) CalIdx.colidx++;



	    /*
	    if(verbosity==kDEBUG||verbosity==kMEDIUM) cout<<"Fill step-check"<<std::endl;
	    hevts->Fill(CalIdx.colidx, CalIdx.dacidx); 	

	    //Fill DACstep_BCO
	    if(verbosity==kDEBUG||verbosity==kMEDIUM) cout<<"Fill DACStep BCO"<<std::endl;
...
int BookHist(std::string base, const FEMCalibParam & calpar, unsigned int layers){
  ...
  snprintf(name,40,"hmeanNhitXStep");
  snprintf(title,80,"Average N of hits for each calibration step");
  hmeanNhitXStep =  new TH2I(name,title,col_steps,-0.5,float(col_steps)-0.5,dacsteps,dacmin,dacmax);

  snprintf(name,40,"hDACTrigResponsePx33");
  snprintf(title,80,"Response map (TrigId vs DACid) of pixel 33");
  hDACTrigResponsePx33 =  new TH2I(name,title,dacsteps,dacmin,dacmax,calpar.triggers, -0.5, float(calpar.triggers)-0.5);
  ...
  for(int i=0; i<4096; i++){
    ...
    calibpxl.push_back(new TH1I(name,title,dacsteps,dacmin,dacmax));
  }
  return 0;
}
...
void NormalizeTo(const FEMCalibParam & calpar){
  ...
  for(int i=0; i<4096; i++){
	  ...
	  if (myvalues[j]>1) cout<<"ERROR: Occupancy > 1    value: "<<values[j]<<"   "<< myvalues[j]<<" "<<i<<" "<<j<<endl;
...
bool sqlselect(int nrun, EPMCCalibParam & epmc1, EPMCCalibParam & epmc2 ){
  ...
  for (int epmc=0; epmc<nrows; epmc++){//EPMC loop
    ...
    if ((Chip1ROIdentifier[epmc]&0xFF000)==0x00000 && (Chip1RWEnable[epmc]&0x80000000) == 0x80000000 ){ //FEM0 is INMAPS and active
      nLayers++;
      //set the layers bit
      activeLayerBit = 0x1 <<((epmcNum[epmc]==1)? 0:4 );
      ep_cal->layers = ep_cal->layers | activeLayerBit; 
      printf("\nFound new active layer 0x%x \n", ep_cal->layers);
      //set the layer type: SPX0
      ep_cal->type0 = 0x0;
    ...
    if ((Chip2ROIdentifier[epmc]&0xFF000)==0x00000 && (Chip2RWEnable[epmc]&0x80000000) == 0x80000000 ) {//FEM1 is INMAPS and active
      nLayers++;
      //set the layers bit
