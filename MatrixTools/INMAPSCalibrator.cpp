...
int main(int argc, char* argv[]){
  if (argc <= 1 ){
    cout << "Usage: INMAPSCalibrator [-v0|1|2] [-evtmax|-e n_of_events] [-rob 1|0] [-purify | -sanitize] [-local triggers DACstart DACstop DACstep Layers nType=3)] <eventfile>" << endl;     
    return -1;
  }
  cout<<"+++++++++++++++INMAPS Calibrator 0.1 by F. Giorgi+++++++++++++++++++++++++"<<endl;
  cout << "Usage: INMAPSCalibrator [-v0|1|2] [-evtmax|-e n_of_events] [-rob 1|0] [-purify | -sanitize] [-local triggers DACstart DACstop DACstep Layers nType=3] <eventfile>" << endl<<"INFO:: -purify skip hits that belongs to wrong columns w.r.t. current col-step"
  ...
  if (SQLmode){
    ...
    if(      EPMCcalpar1.type0==0x5 && EPMCcalpar1.layers==0x1){
      FEMcalpar = EPMCcalpar1.fem0_cp;
      Layers = EPMCcalpar1.layers;
    } else if (EPMCcalpar1.type1==0x5 && EPMCcalpar1.layers==0x2){
      FEMcalpar = EPMCcalpar1.fem1_cp;
      Layers = EPMCcalpar2.layers;
    } else if (EPMCcalpar2.type0==0x5 && EPMCcalpar2.layers==0x10){
      FEMcalpar = EPMCcalpar2.fem0_cp;
      Layers = EPMCcalpar2.layers;
    } else if (EPMCcalpar2.type1==0x5 && EPMCcalpar2.layers==0x20){
      FEMcalpar = EPMCcalpar2.fem1_cp;
      Layers = EPMCcalpar2.layers;
    } else {
      cout<<"ERROR! SQL returned no active INMAPS pixel layer "<<endl;
      return -1;
    }
...
bool sqlselect(int nrun, EPMCCalibParam & epmc1, EPMCCalibParam & epmc2 ){
  ...
  for (int epmc=0; epmc<nrows; epmc++){//EPMC loop
    ...
    if ((Chip1ROIdentifier[epmc]&0xFF000)==0x05000 && (Chip1RWEnable[epmc]&0x80000000) == 0x80000000 ){ //FEM0 is INMAPS and active
      nLayers++;
      //set the layers bit
      activeLayerBit = 0x1 <<((epmcNum[epmc]==1)? 0:4 );
      ep_cal->layers = ep_cal->layers | activeLayerBit; 
      printf("\nFound new active layer 0x%x \n", ep_cal->layers);
      //set the layer type: INMAPS
      ep_cal->type0 = 0x5;
    ...
    if ((Chip2ROIdentifier[epmc]&0xFF000)==0x05000 && (Chip2RWEnable[epmc]&0x80000000) == 0x80000000 ) {//FEM1 is INMAPS and active
      nLayers++;
      //set the layers bit
